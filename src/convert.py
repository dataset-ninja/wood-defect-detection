...
def create_ann(image_path, masks_path, bboxes_path):
    labels = []
...
with open(semantic_map_path) as f:
    ...
    for idx in range(0, len(content), 4):
        ...
        color_to_obj_class[tuple(color_rgb)] = sly.ObjClass(class_name, sly.Bitmap, color_rgb)
        name_to_obj_class[class_name] = sly.ObjClass(class_name + "_bbox", sly.Rectangle, color_rgb)


obj_classes = list(color_to_obj_class.values()) + list(name_to_obj_class.values())
    ...
    for curr_data in all_data:
        ...
        if dir_exists(images_path) and curr_data[:6] == images_folder:
            ...
            for img_names_batch in sly.batched(images_names, batch_size=batch_size):
                ...
                anns_batch = [
                    create_ann(image_path, masks_path, bboxes_path)
